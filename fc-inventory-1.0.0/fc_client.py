...
class FCClient:
    ...
    def get_vms(self, site_uri):
        ...
        # 陈俊杰 - 去掉虚拟机模板
        return self._get_all(f"{site_uri}/vms?isTemplate=false", "vms")
    ...
    # 陈俊杰-增加快照
    def get_vm_snapshots(self, vm_uri):
        """Get snapshots of a VM."""
        data = self._get(f"{vm_uri}/snapshots")
        return self.parse_snapshots(data=data)

    def parse_snapshots(self, data: dict) -> list:
        """
        解析包含嵌套快照信息的JSON字符串，提取所有快照对象到一个列表中。

        该函数会处理两个部分的数据：
        1. 'currentSnapshot'：当前活动的快照。
        2. 'rootSnapshots'：一个快照树，可能包含多层嵌套的子快照。

        Args:
            data (list): 一个包含快照信息。

        Returns:
            list: 一个列表，其中每个元素都是一个快照的字典对象。
                  如果解析失败或输入为空，则返回空列表。
        """
        ...
        # 2. 递归处理 'rootSnapshots' 树
        def _extract_snapshots_recursive(snapshot_nodes: list):
            """一个内部辅助函数，用于递归遍历快照树。"""
            ...
            for snapshot in snapshot_nodes:
                ...
                # 检查是否存在子快照，并递归调用
                if snapshot.get('childSnapshots'):
                    _extract_snapshots_recursive(snapshot['childSnapshots'])

        # 开始从根节点遍历
        if len(data.get('rootSnapshots')) != 0:
            _extract_snapshots_recursive(data['rootSnapshots'])
