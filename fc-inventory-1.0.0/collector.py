...
class InventoryCollector:
            ...
            for site in self.sites:
                ...
                for i, vm in enumerate(vms):
                    ...
                    try:
                        ...
                        # 陈俊杰 - 采集快照信息
                        snaps = self.client.get_vm_snapshots(vm_uri)
                        self.vm_snaps[vm_urn] = snaps
    ...
    def _build_vinfo(self):
        ...
        for vm in self.vms:
            ...
            # 磁盘数量
            row["Disks"] = len(disks)
            row["Create Time"] = _try_paths(merged, ["createTime"])
            row["URN"] = urn
            rows.append(row)
        return rows
    ...
    def _build_vdisk(self):
        ...
        for vm in self.vms:
            ...
            for disk in disks:
                ...
                row["Disk Name"] = _try_paths(disk, ["diskName"])
                row["Dev Name"] = _try_paths(disk, ["devName"])
    ...
    # 陈俊杰
    def _build_vsnapshot(self):
        rows = []
        for vm in self.vms:
            urn = vm.get("urn", "")
            snaps = self.vm_snaps.get(urn, [])
            for snap in snaps:
                row = OrderedDict()
                row["VM Name"] = vm.get("name", "")
                row["Snapshot Name"] = snap.get("name")
                row["Description"] = snap.get("description")
                row["Status"] = snap.get("status")
                row["Type"] = snap.get("type")
                row["URN"] = snap.get("urn")
                row["In Protection Period"] = snap.get("inProtectionPeriod")
                row["Create Time"] = snap.get("createTime")
                rows.append(row)
        return rows
    ...
    def _build_vhost(self):
        ...
        for host in self.hosts:
            ...
            row["Description"] = _try_paths(merged, ["description"])
            row["IP Address"] = _try_paths(merged, ["ip", "ntpIp1", "ntpIp2", "ntpIp3"])
            row["Status"] = _try_paths(merged, ["status"])
            row["Cluster"] = self.cluster_map.get(_try_paths(merged, ["clusterUrn"]), "")
            row["Arch"] = _try_paths(merged, ["arch"])
            row["Os Vendor"] = _try_paths(merged, ["osVendor"])
            row["CPU Vendor"] = _try_paths(merged, ["cpuVendor"])
            row["CPU Cores"] = _try_paths(merged, ["cpuQuantity"])
            row["CPU MHz"] = _try_paths(merged, ["cpuMHz"])
            row["CPU Physical Quantity"] = _try_paths(merged, ["physicalCpuQuantity"])
            row["Total Size (MHz)"] = merged.get("cpuResource", {}).get("totalSizeMHz", "")
            row["Allocated Size (MHz)"] = merged.get("cpuResource", {}).get("allocatedSizeMHz", "")
            row["Running VMs"] = _try_paths(merged, ["runningVmCount"]) or host_vm_count.get(urn, 0)
            row["BMC IP"] = _try_paths(merged, ["bmcIp"])
            row["Maintaining"] = _try_paths(merged, ["isMaintaining"])
            row["CPU Usage"] = _try_paths(merged, ["cpuUsage"])
    ...
    def _build_vcluster(self):
        ...
        for cluster in self.clusters:
            ...
            row["Arch"] = _try_paths(cluster, ["arch"])
            row["HA Enabled"] = _try_paths(cluster, ["isEnableHa", "isHA"])
            row["DRS Enabled"] = _try_paths(cluster, ["isEnableDrs", "isHA"])
            row["DRS Enabled"] = _try_paths(cluster, ["isEnableDrs"])
            row["Auto Adjust Numa"] = _try_paths(cluster, ["isAutoAdjustNuma"])
            row["Mem Overcommit"] = _try_paths(cluster, ["isMemOvercommit"])
            row["Auto Adjust NUMA"] = _try_paths(cluster, ["isAutoAdjustNuma"])
            row["DRS Level"] = cluster.get("drsSetting", {}).get("drsLevel", "")
    ...
    def _build_vdatastore(self):
        ...
        for ds in self.datastores:
            ...
            free = _try_paths(ds, ["freeSizeGB", "freeSpace", "freeCapacityGB", "freeSpaceGB"])
            if not free:
            ...
            try:
                ...
                pass
            row["Used %"] = used_pct
            row["Allocated Capacity (GB)"] = _try_paths(ds, ["usedSizeGB"])
            row["Status"] = _try_paths(ds, ["status"])
            row["Hosts"] = len(ds.get("hosts", []))
            row["Thin Support"] = _try_paths(ds, ["isThin"])
